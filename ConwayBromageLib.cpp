 ...
 */
vector<uint64_t> next(uint64_t nonCompressedKMer, sd_vector<> const& currentCompressedSeq){
    vector<uint64_t> res;
 ...
 */
vector<uint64_t> previous(uint64_t compressedKMer, sd_vector<> const& currentCompressedSeq){
...
vector<uint64_t> successor(uint64_t compressedKMer, sd_vector<> const& currentCompressedSeq){
    vector<uint64_t> succ;
    int currentKMerLen = log(currentCompressedSeq.size()) / log(ALPHABET);
    if(compressedKMer < currentCompressedSeq.size() / 4 && compressedKMer >= 0){
        cout << "the compressed k-mer : " << decode(compressedKMer, currentKMerLen-1) << endl;
        vector<uint64_t> potentialSucc;
        vector<uint64_t> inter;
        potentialSucc = previous(compressedKMer, currentCompressedSeq);
        inter = next(compressedKMer, currentCompressedSeq);
        potentialSucc.insert(potentialSucc.end(), inter.begin(), inter.end());
        cout << "POT : " << endl;
        for(int i = 0 ; i < potentialSucc.size() ; i++){
            cout << decode(potentialSucc[i], currentKMerLen-1) << endl;
        }
        cout << "All potential succ : " << endl;
        for(int i = 0 ; i < potentialSucc.size() ; i++){
            uint64_t reverseSucc = reverseComplementLexico(potentialSucc[i], currentKMerLen-1);
            if(potentialSucc[i] <= reverseSucc){
                succ.push_back(potentialSucc[i]);
                cout << "forward : " << decode(potentialSucc[i], currentKMerLen-1) << endl;
            }else if (potentialSucc[i] > reverseSucc){
                cout << "reverse : " << decode(reverseSucc, currentKMerLen-1) << endl;
        ...
        for (int i = 0; i < currentCompressedSeq.size(); i++) {
            if (currentCompressedSeq[i]) {
                if (decode(i, currentKMerLen).find(sub) != string::npos) {
                    cout << "Found in : " << decode(i, currentKMerLen) << endl;
                    return true;
                }
            }
        }
...
sd_vector<>fromFileToSdVectorWithReverse(string path){
    ...
    if(file){   // File is now open
        ...
        sd_vector<>finalAll = merge(finalSparseRight, finalSparseReverse, myOneLen, myOneLen);  //merging
        file.close();
        return finalAll;
    }else{
...
//Ancient code version, useless now
